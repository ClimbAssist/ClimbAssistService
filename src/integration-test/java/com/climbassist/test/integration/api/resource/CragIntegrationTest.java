...
public class CragIntegrationTest extends AbstractTestNGSpringContextTests {
    ...
    public void getCountry_listsOnlyPublicCrags_whenSomeCragsAndInReviewAndUserIsNotSignedIn() {
        ...
        crag2.setState(State.PUBLIC.toString());
        climbAssistClient.updateCrag(crag2, cookies);
        ApiResponse<Country> apiResponse =
                climbAssistClient.getCountry(country.getCountryId(), RESOURCE_DEPTH, ImmutableSet.of());
        ExceptionUtils.assertNoException(apiResponse);
    ...
    public void updateCrag_removesOptionalParameters_whenOptionalParametersWereOriginallyPresent() {
        ...
        Crag updatedCragWithoutOptionalParameters = Crag.builder()
                ...
                .state(State.PUBLIC.toString())
                .build();
        ApiResponse<UpdateResourceResult> apiResponse =
                climbAssistClient.updateCrag(updatedCragWithoutOptionalParameters, cookies);
        ExceptionUtils.assertNoException(apiResponse);
        assertThat(apiResponse.getData()
                .isSuccessful(), is(true));
        Crag actualCrag = climbAssistClient.getCrag(originalCrag.getCragId(), cookies)
    ...
    public void uploadModels_returnsAuthorizationException_whenUserIsNotSignedIn() {
        ApiResponse<UploadModelsResult> apiResponse =
                climbAssistClient.uploadCragModel("does-not-exist", HIGH_RESOLUTION_MODEL_1, LOW_RESOLUTION_MODEL_1,
                        ImmutableSet.of());
        ExceptionUtils.assertAuthorizationException(apiResponse);
    }
    ...
    public void uploadImage_returnsAuthorizationException_whenUserIsNotSignedIn() {
        ApiResponse<UploadImageResult> apiResponse =
                climbAssistClient.uploadCragImage("does-not-exist", IMAGE_1_JPG, ImmutableSet.of());
        ExceptionUtils.assertAuthorizationException(apiResponse);
    }
    ...
    public void uploadImage_returnsAuthorizationException_whenUserIsNotAdministrator() {
        ApiResponse<UploadImageResult> apiResponse =
                climbAssistClient.uploadCragImage("does-not-exist", IMAGE_1_JPG, cookies);
        ExceptionUtils.assertAuthorizationException(apiResponse);
    }
    ...
    public void uploadImage_uploadsImage() throws IOException {
        testUserManager.makeUserAdministrator(username);
        Crag crag = ResourceManager.getCrag(resourceManager.createCountry(cookies, RESOURCE_DEPTH));
        uploadImage(crag.getCragId(), IMAGE_1_JPG, IMAGE_1_WEBP);
    }
    ...
    public void uploadImage_replacesOldImage_whenCragAlreadyHasAnImage() throws IOException {
        ...
        Crag crag = ResourceManager.getCrag(resourceManager.createCountry(cookies, RESOURCE_DEPTH));
        uploadImage(crag.getCragId(), IMAGE_1_JPG, IMAGE_1_WEBP);
        uploadImage(crag.getCragId(), IMAGE_2_JPG, IMAGE_2_WEBP);
    }
    ...
    public void uploadImage_returnsWebpConverterException_whenImageIsNotAValidJpg() {
        ...
        Crag crag = ResourceManager.getCrag(resourceManager.createCountry(cookies, RESOURCE_DEPTH));
        ApiResponse<UploadImageResult> apiResponse =
                climbAssistClient.uploadCragImage(crag.getCragId(), INVALID_JPG, cookies);
    ...
    public void deleteCrag_returnsAuthorizationException_whenUserIsNotSignedIn() {
        ApiResponse<DeleteResourceResult> apiResponse =
                climbAssistClient.deleteCrag("does-not-exist", ImmutableSet.of());
        ExceptionUtils.assertAuthorizationException(apiResponse);
    }
    ...
    @Test
    public void deleteCrag_deletesModelsAndImages_whenCragHasModelsAndImages() throws IOException {
        testUserManager.makeUserAdministrator(username);
        Crag crag = ResourceManager.getCrag(resourceManager.createCountry(cookies, RESOURCE_DEPTH));
        uploadModels(crag.getCragId(), HIGH_RESOLUTION_MODEL_1, LOW_RESOLUTION_MODEL_1);
        uploadImage(crag.getCragId(), IMAGE_1_JPG, IMAGE_1_WEBP);
        ...
        s3Proxy.assertS3ObjectDoesNotExist(crag.getModel()
                .getModelLocation());
        s3Proxy.assertS3ObjectDoesNotExist(crag.getModel()
                .getLowResModelLocation());
        s3Proxy.assertS3ObjectDoesNotExist(crag.getImageLocation());
        s3Proxy.assertS3ObjectDoesNotExist(crag.getJpgImageLocation());
    }

    private void runGetCragTest(int actualDepth,
        ...
        ApiResponse<Crag> apiResponse = maybeRequestDepth.isPresent() ?
                climbAssistClient.getCrag(crag.getCragId(), maybeRequestDepth.get(), cookies) :
                climbAssistClient.getCrag(crag.getCragId(), cookies);
        ExceptionUtils.assertNoException(apiResponse);
        assertThat(apiResponse.getData(), is(equalTo(crag)));
    }
    ...
    private void uploadModels(String cragId, File highResolutionModel, File lowResolutionModel) throws IOException {
        Crag originalCrag = climbAssistClient.getCrag(cragId, cookies)
                .getData();
        ApiResponse<UploadModelsResult> apiResponse =
                climbAssistClient.uploadCragModel(cragId, highResolutionModel, lowResolutionModel, cookies);
        ExceptionUtils.assertNoException(apiResponse);
        assertThat(apiResponse.getData()
                .isSuccessful(), is(true));
        ...
        s3Proxy.assertS3ObjectEquals(actualCrag.getModel()
                .getModelLocation(), new FileInputStream(highResolutionModel));
        s3Proxy.assertS3ObjectEquals(actualCrag.getModel()
                .getLowResModelLocation(), new FileInputStream(lowResolutionModel));
    }

    private void uploadImage(String cragId, File jpgImage, File webpImage) throws IOException {
        Crag originalCrag = climbAssistClient.getCrag(cragId, cookies)
                .getData();
        ApiResponse<UploadImageResult> apiResponse = climbAssistClient.uploadCragImage(cragId, jpgImage, cookies);
        ExceptionUtils.assertNoException(apiResponse);
        assertThat(apiResponse.getData()
                .isSuccessful(), is(true));
        Crag actualCrag = climbAssistClient.getCrag(cragId, cookies)
        ...
        assertThat(actualCrag.getJpgImageLocation(), is(not(nullValue())));
        originalCrag.setImageLocation(actualCrag.getImageLocation());
        originalCrag.setJpgImageLocation(actualCrag.getJpgImageLocation());
        assertThat(actualCrag, is(equalTo(originalCrag)));

        s3Proxy.assertS3ObjectEquals(actualCrag.getJpgImageLocation(), new FileInputStream(jpgImage));
        s3Proxy.assertS3ObjectEquals(actualCrag.getImageLocation(), new FileInputStream(webpImage));
    }
